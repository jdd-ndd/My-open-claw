...
> **当前状态（v1.1.2）**：全部 9 大核心模块（Gateway、Core、Agents、Tools、Channels、Memory、Skills、Hooks、Services）均已完整实现。客户端模块（Web React 18、TUI Ink 4.x + Textual Python、CLI Commander 12.x）均已开发完成并可独立运行。
...
| CH-003 | QQBot 适配器 | 对接 QQ Bot API，支持文本、图片、命令消息 | C | P0 | 12h | CH-001 | 可收发消息；支持命令；长轮询/Webhook 模式可切 |
| CH-004 | WeChat 适配器 | 对接微信公众平台，支持文本、图片消息 | C | P0 | 10h | CH-001 | 可收发消息；支持消息加密；支持公众号/企业微信 |
...
interface InboundMessageRequest {
  ...
  /** 会话 ID，同一会话内保持一致 */
  sessionId: string;
  /** 渠道标识，如 "webchat"、"qqbot"、"feishu"、"wechat"、"cli" */
  channel: string;
  /** 渠道内的原始会话 ID（如 QQBot groupId） */
...
| `fix` | Bug 修复 | `fix(channels): 修复 QQBot 重连失败问题` |
